...
class Grafo:
    ...
    def criarVertice(self, numVertice):
        # Criar vértice a partir de um valor
        self.__listaAdj[numVertice] = list()

    def inserirAresta(self, verticeOrigem, verticeDestino, peso=None):
        # Inserir aresta de um vértice origem para um vértice destino
        if verticeOrigem not in self.__listaAdj:
            self.criarVertice(verticeOrigem)

        if verticeDestino not in self.__listaAdj:
            self.criarVertice(verticeDestino)
    ...
    def listaAdjacentes(self, vertice):
        ...
        if vertice in self.__listaAdj:
            if self.ponderado():
                return [v[1] for v in self.__listaAdj[vertice]]
            else:
